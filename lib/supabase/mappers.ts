...
export function mapTransferRowToDomain(transfer: Tables<"transfers">): Transfer {
  return {
    ...
    operatorName: transfer.title,
    route: `${transfer.departure_point ?? "Confirm locally"} ↔ ${transfer.arrival_point ?? "Thoddoo harbor"}`,
    description: transfer.description,
    shortDescription: transfer.description,
    duration: transfer.duration ?? "Confirm timing",
    price: transfer.price ?? "Price on request",
    pricingUnit: transfer.price ? "per-person-one-way" : "on-request",
    departurePoint: transfer.departure_point ?? "Confirm locally",
    arrivalPoint: transfer.arrival_point ?? "Thoddoo harbor",
    scheduleNote: transfer.schedule_note ?? "Schedules can change.",
    image: transfer.image_path,
    gallery: [transfer.image_path],
    highlights: transfer.highlights,
    inclusions: transfer.highlights,
    importantInformation: [transfer.schedule_note ?? "Please confirm the latest schedule before booking."],
    availability: {
      provider: "manual",
    ...
    verified: transfer.verification_status === "verified",
    verificationStatus: transfer.verification_status,
    isPublished: transfer.publication_status === "published",
    seo: {
      title: `${transfer.title} | iThoddoo Maldives`,
      description: transfer.description,
    },
  };
}
